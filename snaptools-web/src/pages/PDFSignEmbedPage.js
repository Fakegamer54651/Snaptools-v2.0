...
export default function PDFSignEmbedPage() {
  ...
  // Make body fullscreen for embedded mode
  useEffect(() => {
  ...
  // Auto-load PDF from query parameter
  useEffect(() => {
  ...
  const loadPdfFromFile = async (file) => {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      pdfCanvas.className = "pdf-canvas shadow-md";
    ...
    // Disable default backspace delete
    ...
    // Add floating delete button for selected objects
  ...
  const handleSave = async () => {
    ...
    if (!fileInput && !window.loadedPdfData) {
      alert("Please upload a PDF first.");
      return;
    }
    ...
    if (fileInput) {
      ...
      existingPdfBytes = window.loadedPdfData;
    }
